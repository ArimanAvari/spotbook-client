...
Экран синхронизации подключён к backend:

- `Экспорт на сервер` отправляет локальные группы и карточки со статусами `NOT_SYNCED` и `DELETED`;
- после успешного экспорта Room перезаписывается актуальными серверными данными;
- `Импорт с сервера` загружает группы и карточки текущего пользователя и сохраняет их в Room;
- для MVP серверные данные перезаписывают локальную базу.
